...
Write-Log -Level 'INFO' -Message "輸出目錄：$outDir"
...
foreach ($raw in $isbnList) {
  ...
  if (-not $searchResp) { Write-Log -Level 'ERROR' -Message "  無法取得結果頁（多次重試後失敗）：$searchUrl"; "（請求失敗，無內容）" | Out-File -FilePath (Join-Path $outDir "$isbn.search.error.html") -Encoding utf8; continue }
  ...
  if (-not $detailResp) { Write-Log -Level 'ERROR' -Message "  無法取得細項頁（多次重試後失敗）：$detailUrl"; "（請求失敗，無內容）" | Out-File -FilePath (Join-Path $outDir "$isbn.detail.error.html") -Encoding utf8; continue }
  ...
  if (-not $marcResp) { Write-Log -Level 'ERROR' -Message "  無法取得 MARC 頁（多次重試後失敗）：$marcUrl"; "（請求失敗，無內容）" | Out-File -FilePath (Join-Path $outDir "$isbn.marc.error.html") -Encoding utf8; continue }

  $preText = Get-PreText -Response $marcResp
  if (-not $preText) { Write-Log -Level 'WARN' -Message "  MARC 頁未找到 <pre>，跳過。已輸出原始 HTML 供除錯。"; $marcResp.Content | Out-File -FilePath (Join-Path $outDir "$isbn.marc.nopre.html") -Encoding utf8; continue }

  $norm = ($preText -replace "`r`n|`n|`r", "`r`n").TrimEnd()
  $norm | Out-File -FilePath (Join-Path $outDir "$isbn.txt") -Encoding utf8
  $marcResp.Content | Out-File -FilePath (Join-Path $outDir "$isbn.html") -Encoding utf8
