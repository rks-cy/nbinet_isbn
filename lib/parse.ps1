...
function Test-NoResult {
  ...
  if ($doc -ne $null) {
    try {
      $a = $doc.querySelector('span.briefcitTitle a')
      return ($a -eq $null)
    } catch {
...
function Get-FirstResultLink {
  ...
  if ($doc -ne $null) {
    try {
      # Prefer raw attribute to avoid mshtml 'about:' absolute href; then decode entities
      $anchor = $doc.querySelector('span.briefcitTitle a')
      if ($anchor -ne $null) {
        $href = $anchor.getAttribute('href')
        if (-not $href) { $href = $anchor.href }
        if ($href) {
          $href = HtmlDecode $href
          if ($href -like 'about:*') { $href = $href.Substring(6) }
          return (Build-AbsoluteUrl -BaseOrigin $BaseOrigin -Href $href)
        }
      }
      # 回退：尋找 span.className 含 briefcitTitle
        ...
        for ($i=0; $i -lt $spans.length; $i++) {
          ...
          if ($cls -and ($cls -match '(^|\s)briefcitTitle(\s|$)')) {
            ...
            if ($as -and $as.length -gt 0) {
              ...
              if (-not $href) { $href = $as.item(0).href }
              if ($href) {
                $href = HtmlDecode $href
                if ($href -like 'about:*') { $href = $href.Substring(6) }
                return (Build-AbsoluteUrl -BaseOrigin $BaseOrigin -Href $href)
  ...
  if ($m.Success) { 
    $href = HtmlDecode $m.Groups[1].Value
    if ($href -like 'about:*') { $href = $href.Substring(6) }
    return (Build-AbsoluteUrl -BaseOrigin $BaseOrigin -Href $href)
  }
  return $null
...
function Get-PreText {
  ...
  if ($doc -ne $null) {
    try {
      ...
      if ($pres -and $pres.length -gt 0) {
        $text = $pres.item(0).innerText
        if ($text) { return $text }
      }
    } catch { }
  }
